...
// CleanLabels deletes labels generated by gardener on a label list
func CleanLabels(labels map[string]string) map[string]string {
	var (
		labelList = []string{"shoot.garden.sapcloud.io/status", "shoot.garden.sapcloud.io/unhealthy", "shoot.garden.sapcloud.io/healthy"}
		tmpLabels map[string]string
	)
	tmpLabels = labels
	for _, value := range labelList {
		_, ok := tmpLabels[value]
		if ok {
			delete(tmpLabels, value)
		}
	}
	return tmpLabels
}
